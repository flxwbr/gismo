...
public:
    ...
    {
        pde_ptr = static_cast<const gsPoissonHeterogeneousPde<T> *>(&pde);
        numActive = 0;
    }
                    ...
                    gsQuadRule<T> & rule)
    {
        ...
        rhs_ptr = &pde_ptr->rhs(patchIndex)->piece(0);
                         ...
                         const gsMatrix<T>   & quNodes)
    {
        ...
        // Evaluate the diffusion coefficient A, it is assumed that A is patchwise constant
        pde_ptr->getAlpha()->piece(geo.id()).eval_into(md.points, m_diffCoeff);
                         ...
                         gsVector<T> const & quWeights)
    {
        ...
        localMatrix *= 1.0/m_diffCoeff(0,0);
        ...
        for (index_t k = 0; k<quWeights.rows(); ++k) // loop over quadrature nodes
        {
            ...
            localRhs_ += m_diffCoeff(0,0) * weight * dDimBk * m_phRhsGradV;
        }
...
protected:
    ...
    // Local values of the diffusion coefficient, since we assume A to be patchwise constant
    // it would be sufficient to store a scalar
    gsMatrix<T> m_diffCoeff;
