/** @file gsTestSpaceTimeMajorant.h

@brief A class that contains all the utils for testing the space-time majorant
        for the globally stabilized space-time IgA scheme for the heat equation.
...
namespace gismo {
    ...
    public:
        ...
        gsTestSpaceTimeMajorant(const unsigned _exampleNumber,
                ...
                withMajorantII(_withMajorantII)
        {
            dim = (int) d;
        ...
        void gsSaveToFileKMatrix(bool, gsSpaceTimeAssembler<real_t> &, int refCounter);
    ...
    void gsTestSpaceTimeMajorant<d>::initGivenProblemData() {
        ...
        switch (d) {
            case 2:
                ...
                if (exampleNumber == 2 || exampleNumber == 3 || exampleNumber == 4 ||
                    ...
                    exampleNumber == 16 || exampleNumber == 41 || exampleNumber == 43 || exampleNumber == 45) {
                    ...
                    domainName = "rectangle $(0, 2) \\times (0, 1)$";
                           ...
                           exampleNumber == 35 || exampleNumber == 37 || exampleNumber == 38 || exampleNumber == 39 ||
                           exampleNumber == 40 || exampleNumber == 44) {
                    // --------------------------------------------------------------------------------
                    //  Rectangular domains examples
                    // --------------------------------------------------------------------------------
                    ...
                    patches = *gsNurbsCreator<>::BSplineRectangle(low_x, low_y, upp_x, upp_y);
                    // TODO: discuss with Angelos what to do with this example ?
                    //patches = *gsNurbsCreator<>::BSplineRectangleWithIncreasedMultiplicity(low_x, low_y, upp_x, upp_y);
                    /*
                    real_t upp_x(1.0);
                    gsMultiPatch<> patches;
                    std::string fileSrc = gsFileManager::find( "/Users/smatculevich/Dropbox/science/gismo-2018/devel/filedata/planar/spacetime-x01-t02.xml" );
                    gsReadFile<real_t>(fileSrc, patches);
                    */
                    cFriedrichs = upp_x / PI;   // cFriedrichs <= 1 / (pi * sqrt(l_1^{-2} + ... + l_n^{-2}));
                    domainName = "rectangle $(0, 1) x (0, 2)$";

                    gsInfo << "patches.basis(0)" << patches.basis(0) << "\n";
                    //gsInfo << "patches_.basis(0)" << patches_.basis(0) << "\n";

                    //const int mult = patches.basis(0).knots(0).multiplicity(0);
                    //patches.basis(0).insertKnot(0.5, 1, 2);
                }
                break;
        ...
        switch (exampleNumber) {
            ...
            case 34:
                ...
                uExpr = "if( y < 1, sin(pi*x)*(1 - y)^(1/2), "
                        "           if(y == 1, 0, sin(pi*x)*(y - 1)^(1/2))"
                        ")";
                fExpr = "if( y < 1, -(sin(pi*x)*(2*y*pi^2 - 2*pi^2 + 1))/(2*(1 - y)^(1/2)) , "
                        "           if(y == 1, 0, (sin(pi*x)*(2*y*pi^2 - 2*pi^2 + 1))/(2*(y - 1)^(1/2))  )"
                        ")";
                uDExpr = uExpr;
                break;
            ...
            case 37:
                uExpr = "if( y < 1, sin(pi*x)*(1 - y)^(3/4), "
                        "           if(y == 1, 0, sin(pi*x)*(y - 1)^(3/4))"
                        ")";
                fExpr = "if( y < 1, -(sin(pi*x)*(4*y*pi^2 - 4*pi^2 + 3))/(4*(1 - y)^(1/4)) , "
                        "           if(y == 1, 0, (sin(pi*x)*(4*y*pi^2 - 4*pi^2 + 3))/(4*(y - 1)^(1/4))  )"
                        ")";
                uDExpr = uExpr;
                break;
            ...
            case 39:
                uExpr = "if( y < 1, sin(pi*x)*(1 - y)^(4/5), "
                        "           if(y == 1, 0, sin(pi*x)*(y - 1)^(4/5))"
                        ")";
                fExpr = "if( y < 1, -(sin(pi*x)*(5*y*pi^2 - 5*pi^2 + 4))/(5*(1 - y)^(1/5)) , "
                        "           if(y == 1, 0, (sin(pi*x)*(5*y*pi^2 - 5*pi^2 + 4))/(5*(y - 1)^(1/5))  )"
                        ")";
                uDExpr = uExpr;
                break;

            case 40:
                uExpr = "if( y < 1, sin(pi*x)*(1 - y)^(9/10), "
                        "           if(y == 1, 0, sin(pi*x)*(y - 1)^(9/10))"
                        ")";
                fExpr = "if( y < 1, -(sin(pi*x)*(10*y*pi^2 - 10*pi^2 + 9))/(10*(1 - y)^(1/10)), "
                        "           if(y == 1, 0, (sin(pi*x)*(10*y*pi^2 - 10*pi^2 + 9))/(10*(y - 1)^(1/10))  )"
                        ")";
                uDExpr = uExpr;
                break;

            case 41:
                uExpr = "if( y < 1, sin(pi*x)*(1 - y)^(3/4), 0)";
                fExpr = "if( y < 1, -(sin(pi*x)*(4*y*pi^2 - 4*pi^2 + 3))/(4*(1 - y)^(1/4)), 0)";
                uDExpr = uExpr;
                break;
    ...
    template<unsigned d>
    void gsTestSpaceTimeMajorant<d>::gsCreateResultsFolder(bool save,
    ...
    void gsTestSpaceTimeMajorant<d>::gsRecontructMajorantBasedOnOptimalFlux(int refCounter,
                                                                            gsMultiBasis<real_t> &basisY,
                                                                            int yDegree,
                                                                            gsMatrix<real_t> &yVector,
                                                                            gsMultiPatch<real_t> &mpY,
                                                                            gsVector<index_t> &yDOFs,
                                                                            const gsMultiPatch<real_t> &mpV,
                                                                            const gsField<> &v,
                                                                            const gsMultiPatch<real_t> &mpW,
                                                                            const gsField<> &w,
                                                                            gsVector<real_t> &stopcritVector,
                                                                            const gsFunctionExpr<real_t> &fFunc,
                                                                            const gsFunctionExpr<real_t> &uFunc,
                                                                            gsVector<real_t> &hmaxVector,
                                                                            gsVector<real_t> &hminVector,
                                                                            gsVector<double> &timeAsmblDivDivY,
                                                                            gsVector<double> &timeAsmblVectMassY,
                                                                            gsVector<double> &timeAsmblY,
                                                                            gsMatrix<double> &timeSolvY,
                                                                            gsVector<double> &timeAsmblMaj,
                                                                            gsVector<double> &timeAsmblDeltaHMaj,
                                                                            ...
                                                                            gsVector<real_t> &majIIGapVector,
                                                                            std::vector<real_t> &mdDistr,
        ...
        //const gsPiecewiseFunction<real_t> vPiece = mpW.piece(0); // Alternative approximation of v_t improving mEq in majorant
        ...
        gsInfo << "patches = \n" << divdivAssembler.patches() << "\n";
        gsInfo << "patch = \n" << divdivAssembler.patches().patch(0) << "\n";
...
#pragma omp section
    ...
    void gsTestSpaceTimeMajorant<d>::gsGenerateDivMatrixRhs(int refCounter,
        ...
        if(withMajorant) {
            ...
            gsInfo << "numMappers = " << numMappers << "\n";
            gsInfo << "dualMapper = " << divdivMapper << "\n";
            gsInfo << "dim = " << dim << "\n";
    ...
    template<unsigned d>
    void gsTestSpaceTimeMajorant<d>::gsLogAssemblingSolvingTime(int refCount,
                                    gsVector<double> & timeAsmbDivDivY,
                                    gsVector<double> & timeAsmbY,
                                    gsVector<double> & timeAsmbW,
                                    gsVector<double> & timeAsmbMMY,
    ...
    void gsTestSpaceTimeMajorant<d>::gsGetInitialBasis(int vDegree, int yDegree, int wDegree,
        ...
        // Initialisation for a single-patch domain, id = 0
        ...
        gsLogGeometryBasisData(patches.patch(0), vDegree, yDegree, wDegree, 1);
    ...
    void gsTestSpaceTimeMajorant<d>::gsSaveToFileRefinementIterationInfo(bool save,
        ...
        if (save) {
            ...
            std::string refTag = util::to_string(exampleNumber) + "-refnum-"
                                 + util::to_string(refCounter) + "-outof-"
                                 + util::to_string(refTotal);
            if (isAdaptive){
                gsMesh<> mesh;
                gsTensorBSpline<d, real_t> *geo = dynamic_cast< gsTensorBSpline<d, real_t> * >( &(patches.patch(0)));
...
#pragma omp section
    ...
    template<unsigned d>
    void gsTestSpaceTimeMajorant<d>::gsSaveToFileKMatrix(bool save, gsSpaceTimeAssembler<real_t> &assembler, int refCounter) {
        if (save) {
    ...
    void gsTestSpaceTimeMajorant<d>::gsSolveKhfhSystem(gsSpaceTimeAssembler<real_t>& assembler,
        ...
        if (assembler.numDofs()) {
            ...
            // Get an initial guess for the Krylov method:
            // Construct (by Direct Method) on the first refinement step or
            // fetch from the previous refinement step
            if (refCounter == 0) {
            ...
            // In parallel, run iterative scheme
            #pragma omp parallel sections
            {
                ...
                */
                ...
                #pragma omp section
                {
                    //if (dim == 2) {
                        gsCPUStopwatch clock_2;
                        clock_2.restart();
                        // LDLT is for SPD matrices - doesn't work
                        // QR is for any, rectangular matrices - works slower
                        // LU is for square matrices
                        gsSparseSolver<>::LU solverDirect;
                        solverDirect.compute(assembler.matrix());
