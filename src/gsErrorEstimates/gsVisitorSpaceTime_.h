/** @file gsVisitorSpaceTime_.h

    @brief Heat equation element visitor corresponding to the globally-weighted IgA space-time scheme.
...
/** \brief Visitor corresponding to the globally-weighted IgA space-time scheme
 ...
 */
    ...
    class gsVisitorSpaceTime_
    {
    public:

        /** \brief Constructor for gsVisitorSpaceTime_.
         *
        ...
        gsVisitorSpaceTime_(const gsPde<T> & pde)
        {
                             ...
                             const gsMatrix<T>   & quNodes)
        {
            ...
            theta_ptr->eval_into(md.values[0], thetaVals);
                             ...
                             gsVector<T> const      & quWeights)
        {
            ...
            for (index_t k = 0; k < quWeights.rows(); ++k) // loop over quadrature nodes
            {
                ...
                transformGradients(md, k, basisGrads, ph_basisGrad);
                ...
                transformDeriv2Hgrad(md, k, basisGrads, basis2ndDerivs, ph_basisDeriv2);
                ...
                T h       = element.getCellSize();
                T theta   = thetaVals(0, k);
                T delta_h = theta * h;

                // upwind vector = w = [wh_1 .... wh_N], where N is number of active basis function
                upwindFunc          = basisVals.col(k).transpose() + delta_h * ph_basisTimeGrad;
    ...
    protected:
        ...
        gsMatrix<T> ph_basisTimeGrad, ph_basisSpaceGrad, ph_basisMixedDeriv;
