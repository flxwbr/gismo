...
public:
        ...
        : m_domain(&pde.domain()),side1(s1), side2(s2), isHomogeneous(false)
    {
        ...
        //const gsMagnetostaticPde<T>* ppde = static_cast<const gsMagnetostaticPde<T> *>(&pde);
        if(ppde == NULL)
            isHomogeneous = true;
        else
 ...
 */
    ...
    // assemble on element
    inline void assembleDG(gsDomainIterator<T>    & element1,
                         gsDomainIterator<T>    & element2,
                         gsVector<T>            & quWeights)
    {
        const index_t numActive1 = actives1.rows();
        ...
        for (index_t k = 0; k < quWeights.rows(); ++k) // loop over quadrature nodes
        {
            ...
            switch (side1)
            {
                ...
                case 4: //north
                    ...
                    break;
                default:
                    GISMO_ASSERT(d==2, "The dimension is expected to be 2!");
            }
    ...
    inline void localToGlobal(const int patchIndex1,
    ...
    void localToGlobalIETI(const gsDofMapper  & mapper,
                           const gsMatrix<T>     & eliminatedDofs,
                           const gsMatrix<unsigned>& activeExtra,
                           gsSparseMatrix<T>     & sysMatrix,
                           gsMatrix<T>           & rhsMatrix)
    {
    ...
    // assemble on element
    inline void assembleSingleSideDG(gsDomainIterator<T>    & element1,
                                   gsDomainIterator<T>    & element2,
                                   gsVector<T>            & quWeights)
    {
        const index_t numActive1 = actives1.rows();
        ...
        for (index_t k = 0; k < quWeights.rows(); ++k) // loop over quadrature nodes
        {
            // Compute the outer normal vector from the parameter domain
            unormal.resize(d);
            switch (side1)
            {
                case 1: // west
     ...
     * @param activesParam2
     */
    void getActives(gsMatrix<unsigned>*& activesParam1, gsMatrix<unsigned>*& activesParam2)
    {
