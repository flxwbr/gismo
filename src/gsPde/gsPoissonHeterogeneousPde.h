...
template<class T>
class gsPoissonHeterogeneousPde : public gsPoissonPde<T>
{
public:
    ...
    const gsFunction<T> *    rhs(index_t i)      const { return &m_rhs.piece(i); }
    ...
    virtual gsPde<T>* restrictToPatch(unsigned np) const
    {
        ...
        newRhs.addPiece(m_rhs.piece(0));//newRhs.addPiece(m_rhs.piece(np));
        return new gsPoissonHeterogeneousPde<T>(m_domain.patch(np),bc,newRhs,funs);
    }
